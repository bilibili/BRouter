package com.bilibili.brouter.common.analysis
    ...
    fun analyze() {
        ...
        val walker = CachingDirectedGraphWalker(false, object : DirectedGraph<Node, Nothing> {
            override fun getNodeValues(
                ...
                node.serviceDependencies.forEach {
                    val dependency = services[it.className to it.serviceName]
                    check(dependency != null || it.optional || allowMiss) {
                        "(${it.className}, ${it.serviceName}) that $node dependsOn does not exists."
                    }
                    if (dependency != null) {
                        connectedNodes += dependency.toNode()
                    }
                }
        ...
        libs.parallelStream()
            .flatMap {
                ...
                it.services.forEach { serviceMeta ->
                    serviceMeta.serviceTypes.forEach { serviceType ->
                        putService(
                            serviceType to serviceMeta.serviceName,
                            moduleName to serviceMeta
                        )
                    }
                    nodes += ServiceNode(moduleName, serviceMeta)
                }
...
internal class TaskNode(override val moduleName: String, val taskMeta: TaskMeta) :
    Node {
    override val serviceDependencies: List<ServiceDependency>
        get() = taskMeta.constructorParams
    override val taskDependencies: List<String>
